...
std::vector<observables_struct_t> create_observables_pol(float pe = 0.8, float pp = 0.3)
{
    ...
    for (int imodel = 0; imodel < theory.size(); imodel++)
    {
        for (int ienergy = 0; ienergy < 6; ienergy++)
        {
            ...
            for (int iflav = 4; iflav < 6; iflav++)
            {
                ...
                newobs.energy = theory.at(imodel).model_values[ienergy].energy;
                newobs.flav = iflav;
                newobs.Cross_L = ObsCross(imodel, ienergy, iflav, -pe, pp);
                newobs.Cross_R = ObsCross(imodel, ienergy, iflav, pe, -pp);
                newobs.Cross_unpol = ObsCross(imodel, ienergy, iflav, 0, 0);
                newobs.R_L = ObsR(imodel, ienergy, iflav, -pe, pp);
                newobs.AFB_L = ObsAFB(imodel, ienergy, iflav, -pe, pp);
                newobs.R_R = ObsR(imodel, ienergy, iflav, pe, -pp);
                newobs.AFB_R = ObsAFB(imodel, ienergy, iflav, pe, -pp);
                newobs.R_unpol = ObsR(imodel, ienergy, iflav, 0, 0);
                newobs.AFB_unpol = ObsAFB(imodel, ienergy, iflav, 0, 0);
                observables.push_back(newobs);
            }
...
TH2F * AFB(int quark_under_test, float energy_under_test, std::vector<observables_struct_t> observables)
{

    TH2F *h_AFB=  new TH2F(TString::Format("h_AFB_q%i_en%i", quark_under_test, int(energy_under_test)), TString::Format("h_AFB_q%i_en%i", quark_under_test, int(energy_under_test)), 10, -0.5, 9.5, 6, -0.5, 5.5);
    ...
    { // loop for reference model
        // this could be done with iterators, but this is good enough...

        if (observables.at(iobs).flav != quark_under_test)
            continue;
        if (observables.at(iobs).energy != energy_under_test)
            continue;
        ...
        // polarized Beams
        // obnservables
        float AFB_L = observables.at(iobs).AFB_L;
        float AFB_R  = observables.at(iobs).AFB_R;
        float AFB_unpol = observables.at(iobs).AFB_unpol;

        float R_L = observables.at(iobs).R_L;
        float R_R  = observables.at(iobs).R_R;
        float R_unpol = observables.at(iobs).R_unpol;


        h_AFB->SetBinContent(id1 + 1, 1, AFB_L);
        h_AFB->SetBinContent(id1 + 1, 2, AFB_R);
        h_AFB->SetBinContent(id1 + 1, 3, AFB_unpol);
        h_AFB->SetBinContent(id1 + 1, 4, R_L);
        h_AFB->SetBinContent(id1 + 1, 5, R_R);
        h_AFB->SetBinContent(id1 + 1, 6, R_unpol);
...
std::vector<TH2F *> chi2_ndf_models(int quark_under_test, float energy_under_test, std::vector<observables_struct_t> observables)
{
    ...
    { // loop for reference model
        ...
        for (int jobs = 0; jobs < observables.size(); jobs++)
        {
            ...
            if (observables.at(jobs).flav != quark_under_test)
                continue;
            if (observables.at(jobs).energy != energy_under_test)
                continue;

            int iquark = observables.at(iobs).flav;
            if (iquark == 5)
                iquark = 6; // index to read properly the uncertainties
            int index_energy = 0;
            if (observables.at(iobs).energy == 500)
                index_energy = 1;
    ...
    { // loop for reference model
        ...
        {
            for (int i = 0; i < 8; i++)
                if (h_chi2[i]->GetBinContent(jobs + 1, iobs + 1) > h_chi2[i]->GetBinContent(iobs + 1, jobs + 1))
                    h_chi2[i]->SetBinContent(iobs + 1, jobs + 1, h_chi2[i]->GetBinContent(jobs + 1, iobs + 1));
        }
    }
    ...
    {
        if (i == 3 || i == 7)
            continue; // I SKIP THE UNPOL CASE !!
